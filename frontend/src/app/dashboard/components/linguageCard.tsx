...
const ICONES: Record<string, React.ReactNode> = {
  ...
  ie: <Brain className="size-10" />,
  produtividade: <Zap className="size-10" />,
  networking: <Users className="size-10" />,
};
...
export function LinguageCard({ data }: { data: JornadaResponse }) {
  ...
  return (
    <article
      className="bg-card p-5 rounded-xl flex flex-col gap-5 border border-border select-none cursor-pointer hover:scale-[1.03] transition-all duration-200 ease-in-out"
      onMouseEnter={(e) => {
    ...
    >
      ...
      <div className="w-full h-2 bg-muted rounded-full relative mt-auto">
        <div
          ...
          style={{ backgroundColor: data.linguagem.cor, width: `${data.progresso_percent}%` }}
        />
